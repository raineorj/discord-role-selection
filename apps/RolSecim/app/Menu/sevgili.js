...
class RolKatilimci extends ButtonCommand {
    ...
    async run(client, interaction, data) {
       // if(!interaction.member.roles.cache.has(data.roles["family"])) return interaction.reply({embeds: [new Discord.MessageEmbed().setDescription(`Bu komutu kullanabilmek için <@&${data.roles["family"]}> rolüne sahip olmalısın!`)],  ephemeral: true});
        ...
        if (interaction.values.includes("sevgilim_temizle") && interaction.values.length === 1) {
            return await mentioned.roles.remove(Object.keys(data.roles).filter(key => key.startsWith("sevgilim_")).map(key => data.roles[key]));
        } else if (interaction.values.includes("sevgilim_temizle")) {
        ...
        await mentioned.roles.remove(Object.keys(data.roles).filter(key => key.startsWith("sevgilim_")).map(key => data.roles[key]));
